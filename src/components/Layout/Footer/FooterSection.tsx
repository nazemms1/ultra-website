...
export default function FooterSection({ data, statsData }: { data?: any; statsData?: any }) {
  ...
  const bio = hasApiData ? publicDataMap.bio : "We provide cutting-edge solutions for businesses seeking to optimize their operations.";
  const rawLogo = publicDataMap.logo;
  const logoUrl = hasApiData ? getMediaUrl(rawLogo) : "/images/logo/logo-ultra.svg";
  
  const copyright = hasApiData ? publicDataMap.copyright_information : "© 2026 Ultrawares. All rights reserved.";
  ...
  const processedSocials: { href: string; label: string; iconUrl: string | null; icon: ReactNode }[] = hasApiData
